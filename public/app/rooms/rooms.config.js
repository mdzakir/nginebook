...
.factory('ManageRooms', function ($http, $q, apiEndPoint, AppContext, User) {
    return {
        getRooms : function(){
        ...
        deleteRoom: function (params, callback) {
            var post_url = apiEndPoint + '/room/updateStatus?hotel_id='+User.getHotelID()+'&room_id='+params.room_id+'&status=3';
            $http.get(post_url)
                .then(function () {
                    callback();
                });
